const comConfig = {
  IcoWarn: {
    titleH4: {zn: '万维链Token购买申明', en: 'Wanchain Tokens Purchase Statement'},
    titleP: {zn: '请仔细阅读如下内容，并确保您理解了足够的信息', en: 'Please read the following carefully and confrm you understand enough information'},
    checkIn: {
      one: {
        zn: '请点击确定您不是美国或者新加坡公民、居民或者实体，并切不代表任何美国或新加坡人购买WAN Tokens。',
        en: 'Check here to confim that you are not a U.S. and Singapore citizen, resident or entity nor are you purchasing WAN Tokens or signing on behalf of a U.S. and Singapore Person.'
      },
      two: {
        zn: '请点击确定您已经阅读、理解并且同意了',
        en: 'Check here to comfirm that you have read, understand and agree to the terms of the'
      }
    },

  },
  Footer: {
    footEnd: {zn: '©wanchain 2017 版权所有 all rights reserved', en: '©WANCHAIN FOUNDATION LTD 2017 all rights reserved'},
  }
};

export default comConfig;
